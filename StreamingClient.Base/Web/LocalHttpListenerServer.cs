    ...
    public abstract class LocalHttpListenerServer
    {
        ...
        protected string GetRequestParameter(HttpListenerContext listenerContext, string parameter)
        {
            if (listenerContext.Request.RawUrl.Contains(parameter))
            {
                ...
                if (startIndex >= 0)
                {
                    string token = listenerContext.Request.RawUrl[(startIndex + searchString.Length)..];
        ...
        protected async Task<string> GetRequestContentAsync(HttpListenerContext listenerContext)
        {
        ...
        /// <returns></returns>
        protected async Task CloseConnectionAsync(HttpListenerContext listenerContext, HttpStatusCode statusCode, string content)
        {
