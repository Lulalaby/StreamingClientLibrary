...
namespace StreamingClient.Base.Services
{
    ...
    public abstract class OAuthRestServiceBase
    {
        ...
        {
            using AdvancedHttpClient client = await GetHttpClientAsync();
            try
            {
        ...
        {
            using AdvancedHttpClient client = await GetHttpClientAsync(autoRefreshToken);
            try
            {
                client.RateLimitUpdateOccurred += Client_RateLimitUpdateOccurred;
        ...
        protected virtual async Task<AdvancedHttpClient> GetHttpClientAsync(bool autoRefreshToken = true)
        {
            OAuthTokenModel token = await GetOAuthToken(autoRefreshToken);
            return token != null ? new AdvancedHttpClient(GetBaseAddress(), token) : new AdvancedHttpClient(GetBaseAddress());
        }

        private void Client_RateLimitUpdateOccurred(object sender, HttpRequestRateLimits rateLimits) => RateLimitUpdateOccurred(this, rateLimits);
    }
}
