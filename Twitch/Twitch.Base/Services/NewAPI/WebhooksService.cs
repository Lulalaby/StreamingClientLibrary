...
namespace Twitch.Base.Services.NewAPI
{
	...
	public class WebhooksService : NewTwitchAPIServiceBase
	{
		...
		public async Task<IEnumerable<WebhookSubscriptionModel>> GetWebhookSubscriptionsAsync(int maxResults = 100)
		{
			...
			return await GetPagedDataResultAsync<WebhookSubscriptionModel>("webhooks/subscriptions", maxResults);
		}
		...
		public async Task SubscribeWebhookTopicAsync(string callback, string topic, int lease_seconds, string secret)
		{
			...
			await ProcessWebhookTopicRequestAsync(callback, "subscribe", topic, lease_seconds, secret);
		}
		...
		/// <returns>An awaitable task</returns>
		/// <exception cref="HttpRestRequestException">Throw in the event of a failed request</exception>
		public async Task UnsubscribeWebhookTopicAsync(string callback, string topic, string secret)
		{
			Validator.ValidateString(callback, "callback");
			Validator.ValidateString(callback, "topic");
			await ProcessWebhookTopicRequestAsync(callback, "unsubscribe", topic, 0, secret);
		}
		...
		/// <returns>An awaitable task</returns>
		/// <exception cref="HttpRestRequestException">Throw in the event of a failed request</exception>
		private async Task ProcessWebhookTopicRequestAsync(string callback, string mode, string topic, int lease_seconds, string secret)
		{
			HttpResponseMessage response = await PostAsync("webhooks/hub", AdvancedHttpClient.CreateContentFromObject(new WebhookSubscriptionRegistrationModel(callback, mode, topic, lease_seconds, secret)));
